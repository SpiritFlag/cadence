...
test("빈 DB에 마이그레이션을 적용하면 테이블이 생긴다", () => {
  ...
  expect(tableNames(db)).toEqual(["_migrations", "deps", "issues", "proposals", "repos"]);
});
...
test("두 번 적용해도 같다", () => {
  ...
  expect(first).toEqual(["001_init.sql", "002_deps.sql", "003_proposals.sql", "004_proposal_repo.sql"]);
...
test("004는 기존 제안을 담긴 이슈의 레포로 채운다", () => {
  ...
  expect(migrate(db)).toEqual(["004_proposal_repo.sql"]);
