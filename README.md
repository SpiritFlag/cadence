...
bun run start
```
브라우저에서 `localhost:4747`을 연다. 상단 입력칸에 `owner/name`을 넣고 추가하면 그 레포의 열린 이슈가 그래프에 카드로 보인다. 레포가 여럿이면 상단 레포 칩을 눌러 볼 레포를 고른다. 카드 아래 점을 끌어 다른 카드 위 점에 놓으면 "막는다" 선이 생긴다. 서로 막는 선은 빨갛다. [그래프 생성]을 누르면 claude가 이슈 본문을 읽고 순서가 있는 이슈 사이에 점선을 긋는다. 손으로 그은 선을 지우거나 뒤집자는 제안, 사용자가 지운 점선을 다시 긋자는 제안은 [승인]해야 반영된다. [제안]을 누르면 패키지마다 그 이슈들이 그래프에서 박스로 묶이고, 박스끼리 순위대로 파랑 선이 이어진다. 이 선은 보기용이라 "막는다" 선이 아니다.
